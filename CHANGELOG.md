...
### 补充（v1.49.0 交付后，2026-09-25）
- **开发日志数据同步**：将现代版 v1.46.0 ~ v1.49.0 四轮改动按现有格式追加写入主页面开发历程 LOG 数据源 `scripts/history.js`（`STAGES` V3.0 阶段末尾，日期 2026-09-25），主页面 LOG 弹窗 V3.0 阶段由 55 条增至 59 条版本记录；`scripts/modern.js` 的 `DEV_LOG_STAGES` 同步重新提取（两处同源一致）。
- **验证**：`jsc` 双文件语法 OK；CDP 双页 PASS——主页面 LOG 弹窗（开启动画完成后）含 v1.46.0 / v1.47.0 / v1.48.0 / v1.49.0 且 V3.0 旧记录（V3.44.3）仍在；现代版弹窗 59 条、末行 v1.49.0。
- 备注：主页面 LOG 弹窗内容在开启动画结束后渲染，验证需等待动画完成后再读 DOM。
...
- **开发历程弹窗**（`history.js` V3.0 阶段）：09-24 的新条目（V3.44.2 / V3.44.1 / V3.44.0 / V3.43.0 / V3.42.10 / V3.42.9）此前被插在 09-20 的条目（V3.42.5 ~ V3.42.8）之前，弹窗按数组顺序渲染导致版本列表时间倒挂。已按日期升序、同日按版本号升序完整重排（V3.22 → V3.44.2），并确认后续新增版本一律追加到数组末尾保持时间顺序。
- **项目板块成长史**：并列项按实际时间排序——音游展示区（09-18）→ 开发历程 LOG（09-20）→ 反馈墙（09-24），不再是「反馈墙 / 音游展示区 / 开发历程 LOG」。
- 同步更新 `scripts/i18n.js` 中英对照；版本 → 1.44.3；BIOS v1.44.3；history.js 新增 V3.44.3 条目（追加在数组末尾）。

### 涉及文件
| 文件 | 类型 | 说明 |
| --- | --- | --- |
| `scripts/history.js` | 修改 | V3.0 阶段 versions 按时间先后重排 + 新增 V3.44.3 条目 |
| `index.html` | 修改 | 项目板块成长史并列项按时间排序；版本号 → 1.44.3 |
| `scripts/i18n.js` | 修改 | 成长史中英对照同步 |
| `scripts/main.js` | 修改 | BIOS → v1.44.3 |

---

## [v1.44.2] 内容：项目板块写入迭代历史（历史）
...
### 说明
- 把未写入网站的迭代历史补进**项目板块**：MISSION_01「本个人主页」的全息简报此前一直是 AI 生成占位文案（代码注释亦标注「详细文案仍为占位，请按实际情况调整」），本次更新为真实迭代历史：
  - 功能集合：自我介绍、技能进度、项目档案、数字孪生问答 + 访客反馈墙、音游展示区、开发历程 LOG、移动端适配等迭代新增能力；
  - 成长史里程碑：V3.22 上线 GitHub Pages → 反馈墙 / 音游展示区 / 开发历程 LOG → v1.44.0 移动端菜单 → v1.44.1 LOG 修复 → 点击 LOG 查看全部版本（完整版本史仍由 LOG 弹窗 STAGES 承载）。
- 同步更新 `scripts/i18n.js` 中英对照文案；版本 → 1.44.2；BIOS v1.44.2；history.js 新增 V3.44.2 条目。

### 涉及文件
| 文件 | 类型 | 说明 |
| --- | --- | --- |
| `index.html` | 修改 | MISSION_01 全息简报占位文案 → 真实迭代历史（功能集合 + 成长史里程碑） |
| `scripts/i18n.js` | 修改 | 同步中英文对照（简报描述 + 成长史） |
| `index.html` / `scripts/main.js` | 修改 | 版本号与引用 → 1.44.2 |
| `scripts/history.js` | 修改 | 新增 V3.44.2 条目 |

---

## [v1.44.1] 修复：手机端 LOG 按钮被全息投影面板遮挡（历史）
...
### 新增
- 移动端（≤640px）：右侧操作台三枚板块按钮（音乐 / 反馈 / 反馈墙）收进一枚主按钮，点击展开三枚按钮（带依次弹出动画），再点收起；返回顶部保持独立常显（[AI-GEN]）。
- 桌面端布局完全不变：主按钮隐藏，三枚板块按钮照常直接显示。
- 交互细节：主按钮 `+` 图标展开时旋转 45° 变 `×`；点击任一板块按钮触发其功能后自动收起菜单；点击菜单外部任意区域收起；`aria-expanded` / `aria-haspopup` 无障碍标注，双语 aria（[AI-GEN]）。
- **版本**：主页版本号 → 1.44.0；`style.css` / `main.js` / `stickman.js` / `history.js` / `feedback.js` / `feedback-wall.js` / `i18n.js` 引用 `?v=1.44.0`；`main.js` BIOS → v1.44.0；`history.js` 新增 V3.44.0 条目。

### 涉及文件
| 文件 | 类型 | 说明 |
| --- | --- | --- |
| `index.html` | 修改 | 操作台新增移动端主按钮（`#menuBadge`，桌面端隐藏）；版本号与引用 → 1.44.0 |
| `styles/style.css` | 修改 | `.menu-badge` 样式（同族毛玻璃黄线，桌面隐藏）；移动端折叠 / 展开逻辑 + 依次弹出动画 |
| `scripts/main.js` | 修改 | 主按钮展开收起交互（含外部点击收起、子按钮触发后收起）；BIOS → v1.44.0 |
| `scripts/i18n.js` | 修改 | 新增「打开功能菜单 / Open menu」双语翻译 |
| `scripts/history.js` | 修改 | 新增 V3.44.0 条目 |

### 验证方式
- `main.js` / `i18n.js` V8 `--check` 语法通过；页面 HTTP 200。
- CDP 浏览器实测（移动视口 390×844）：桌面视口下主按钮隐藏、三按钮直显；移动视口下仅主按钮 + 返回顶部；点击主按钮三枚按钮依次弹出、再点收起、点外部收起；点音乐/反馈/反馈墙分别正常打开浮窗/弹层且菜单自动收起；中英双语 aria 正常。

---

## [v1.43.0] 新增：反馈墙（公开反馈展示 + 点赞 + 评论 + 主人下架）（历史）
